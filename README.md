...
with `v`, such as `v3.02.6`. The workflow builds both architectures, creates a
...
The one-line commands below install the current `v3.02.6` packaging release for
...
```sh
mkdir -p "$HOME/.local/bin" && curl -fL "https://github.com/9M2PJU/netscape-communicator-3-02-for-linux/releases/download/v3.02.6/netscape-communicator-3.02-linux-amd64.AppImage" -o "$HOME/.local/bin/netscape-communicator.AppImage" && chmod +x "$HOME/.local/bin/netscape-communicator.AppImage" && APPIMAGE_EXTRACT_AND_RUN=1 "$HOME/.local/bin/netscape-communicator.AppImage"
```

### Debian-based distributions

```sh
curl -fL "https://github.com/9M2PJU/netscape-communicator-3-02-for-linux/releases/download/v3.02.6/netscape-communicator-3.02-linux-amd64.deb" -o /tmp/netscape-communicator-3.02-linux-amd64.deb && sudo apt install -y /tmp/netscape-communicator-3.02-linux-amd64.deb
```
...
```sh
curl -fL "https://github.com/9M2PJU/netscape-communicator-3-02-for-linux/releases/download/v3.02.6/netscape-communicator-3.02-1.x86_64.rpm" -o /tmp/netscape-communicator-3.02-1.x86_64.rpm && sudo dnf install -y /tmp/netscape-communicator-3.02-1.x86_64.rpm
```
...
On Arch Linux or CachyOS, where Debian and RPM packaging tools may not be
installed, build only the runnable AppImage:
...
```sh
git tag -a v3.02.6 -m "Netscape Communicator 3.02.6 Linux packaging"
git push origin v3.02.6
```
...
requires a pushed tag matching `v*`, such as `v3.02.6`. The tag workflow must
